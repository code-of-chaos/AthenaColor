...
color3 = {repr(color3:=HSV(h=360,s=.5,v=0))}
color4 = {repr(color4:=HSL(h=360,s=.5,l=0))}
color5 = {repr(color5:=CMYK(c=1,m=.75,y=.25,k=0))}
color6 = {repr(color6:=RGBA(r=255,g=127,b=63,a=0))}
color7 = {repr(color7:=HEXA(hex_value="#ff7f3f00"))}
"""
    )
...
{repr(color3 := color1 - color2)} = {repr(color1)} - {repr(color2)}
"""
    )
...
result -> {repr(color3:=color1-color2)} = {repr(color1)} - {repr(color2)}
...
result -> {repr(color4:=color1-(color2/color3))} = {repr(color1)} - ({repr(color2)} / {repr(color3)})
