...
class ColorObjects_HSV(unittest.TestCase):
    ...
    def test_dunder_HEX(self):
        ...
        # math
        self.assertEqual(
            round(color + HEX("#abcdef"),init.decimalPlaces),
            HSV(210.25,0.785,1)
        )
        self.assertEqual(
            round(color - HEX("#00bb55"),init.decimalPlaces),
            HSV(0,0,0.017)
        )
        self.assertEqual(
            round(color * HEX("#020302"),init.decimalPlaces),
        ...
        self.assertEqual(
            round(color // HEX("#050709"),init.decimalPlaces),
            HSV(0.0,1.0,1)
        )
        self.assertEqual(
            round(color ** HEX("#020101"),init.decimalPlaces),
            HSV(1.0,0.707,0.998)
        )
        self.assertEqual(
            round(color % HEX("#121664"),init.decimalPlaces),
            HSV(0.25,0.5,0.358)
        )
    ...
    def test_dunder_HSL(self):
        ...
        # math
        self.assertEqual(
            round(color + HSL(0.125,0.5,0.1),init.decimalPlaces),
            HSV(0.25,1,0.899)
        )
        self.assertEqual(
            round(color - HSL(0.125,0.5,0.1),init.decimalPlaces),
        ...
        self.assertEqual(
            round(color * HSL(0,0,0.01),init.decimalPlaces),
            HSV(0.0,0.0,0.009)
        )
        self.assertEqual(
            round(color / HSL(180,1,.01),init.decimalPlaces),
            HSV(0.001,0.5,1)
        )
        self.assertEqual(
            round(color // HSL(180,1,.01),init.decimalPlaces),
        ...
        self.assertEqual(
            round(color ** HSL(0,0,0.006),init.decimalPlaces),
            HSV(1.0, 1.0, 0.998)
        )
        self.assertEqual(
            round(color % HSL(180,0.2,0.1),init.decimalPlaces),
    ...
    def test_dunder_CMYK(self):
        ...
        self.assertEqual(
            round(color ** CMYK(1,0,0,0),init.decimalPlaces),
            HSV(0.0,0.5,0.75)
        )
        self.assertEqual(
